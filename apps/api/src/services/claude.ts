...
// CommonGround Bently system prompt
...
export async function bentlyRewrite(
  userMessage: string,
  style: 'gentle' | 'direct' | 'collaborative'
): Promise<string> {
  const styleGuides = {
...
Return ONLY the rewritten message, nothing else. Keep it concise.`,
    messages: [
      {
        role: 'user',
        content: `Original message: "${userMessage}"\n\nRewrite it as: ${style}`,
      },
    ],
  })
...
Keep it to 3-4 sentences.`,
    messages: [
      {
        role: 'user',
        content: `What does today hold for a ${zodiac1} and ${zodiac2} couple?`,
      },
    ],
  })
